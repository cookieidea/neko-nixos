...
# 系统 profile 与标准路径里查找 shutter 音效；找不到则静默（不报错）。
SOUND="$(ls /run/current-system/sw/share/sounds/freedesktop/stereo/camera-shutter.oga \
           /usr/share/sounds/freedesktop/stereo/camera-shutter.oga 2>/dev/null | head -1)"
